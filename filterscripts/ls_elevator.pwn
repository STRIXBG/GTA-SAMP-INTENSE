...
public OnObjectMoved(objectid)
{
	...
	if(objectid == Obj_Elevator)   // The elevator reached the specified floor.
	{
	    ...
	    Label_Elevator	= Create3DTextLabel("{CCCCCC}Натисни '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' за да използваш асансьора", 0xCCCCCCAA, 1784.9822, -1302.0426, z - 0.9, 4.0, 0, 1);
...
stock Elevator_Initialize()
{
	...
	Label_Elevator          = Create3DTextLabel("{CCCCCC}Натисни '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' за да използваш асансьора", 0xCCCCCCAA, 1784.9822, -1302.0426, 13.6491, 4.0, 0, 1);
	...
	for(new i; i < sizeof(Obj_FloorDoors); i ++)
	{
		...
		format(string, sizeof(string), "{CCCCCC}[%s]\n{CCCCCC}Натисни '{FFFFFF}~k~~CONVERSATION_YES~{CCCCCC}' за да извикаш асансьора", FloorNames[i]);
...
stock ShowElevatorDialog(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_ID, DIALOG_STYLE_LIST, "Асансьор", string, "Избери", "Изход");
